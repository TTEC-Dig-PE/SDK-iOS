...
+ (ECSThemeEngine *) sharedInstance
{
    static dispatch_once_t onceToken;
    
    dispatch_once(&onceToken, ^{
        sharedInstance = [[ECSThemeEngine alloc] init];
    });
                     ...
                     fromArray:(NSArray *) gradientArray
{
    ...
    for (NSDictionary *colElement in gradientArray) 
    {
        ...
        [gradientColorsArray addObject:(id)[ECSThemeEngine colorFromString:color].CGColor];
...
{
    NSDictionary *themeDictionary = [[ECSThemeEngine sharedInstance] themeDictForType:themeElementType 
    ...
    CGContextSaveGState(context);
    {
        if (shadowDict)
        {
            ...
            UIColor *shadowColor = [ECSThemeEngine colorFromString:[shadowDict elementInThemeDictOfGenericType:PMThemeColorGenericType]];
            [shadowColor set];
        }
        ...
        if (isGradient)
        {
            ...
            [ECSThemeEngine drawGradientInContext: context
        ...
        {
            CGContextSetTextDrawingMode(context, kCGTextFill);
            [[ECSThemeEngine colorFromString:colorObj] setFill];
        ...
        inContext:(CGContextRef) context
{
    ...
    }
    {
        ...
        if (stroke)
        {
            ...
            UIColor *strokeColor = [ECSThemeEngine colorFromString:strokeColorStr];
...
{
    return [[[ECSThemeEngine sharedInstance] themeDictForType:type 
...
- (NSDictionary *) themeDictForType:(PMThemeElementType) type 
                            subtype:(PMThemeElementSubtype) subtype
{
    NSDictionary *result = [sharedInstance.themeDict objectForKey:[ECSThemeEngine keyNameForElementType:type]];
    
    if (subtype != PMThemeNoSubtype)
    {
        result = [result objectForKey:[ECSThemeEngine keyNameForElementSubtype:subtype]];
    }
...
{
    return [self objectForKey:[ECSThemeEngine keyNameForGenericType:type]];
}
...
- (UIFont *) pmThemeGenerateFont
{
    ...
    if (!size)
    {
        return [ECSThemeEngine sharedInstance].defaultFont;
    }
