...
@property (nonatomic, strong) ECSDaysView *daysView;
@property (nonatomic, strong) ECSSelectionView *selectionView;
@property (nonatomic, assign) CGRect initialFrame;
...
@implementation ECSCalendarView
{
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    self.selectionView = [[ECSSelectionView alloc] initWithFrame:CGRectInset(self.bounds, -kPMThemeInnerPadding.width, -kPMThemeInnerPadding.height)];
    [self addSubview:self.selectionView];

    self.daysView = [[ECSDaysView alloc] initWithFrame:self.bounds];
...
- (void)drawRect:(CGRect)rect
{
    ...
    UIFont *dayFont = [[[ECSThemeEngine sharedInstance] elementOfGenericType:PMThemeFontGenericType
                                                                      subtype:PMThemeMainSubtype
                                                                         type:PMThemeDayTitlesElementType] pmThemeGenerateFont];
    UIFont *monthFont = [[[ECSThemeEngine sharedInstance] elementOfGenericType:PMThemeFontGenericType
                                                                      subtype:PMThemeMainSubtype
    ...
    for (int i = 0; i < dayTitles.count; i++) 
    {
        ...
        [[ECSThemeEngine sharedInstance] drawString:dayTitle
                                          withFont:dayFont
    ...
    [[ECSThemeEngine sharedInstance] drawString:monthTitle
                                      withFont:monthFont
                                        inRect:textFrame
    ...
    NSDictionary *arrowSizeDict = [[ECSThemeEngine sharedInstance] elementOfGenericType:PMThemeSizeGenericType
                                                                               subtype:PMThemeMainSubtype
                                                                                  type:PMThemeMonthArrowsElementType];

    NSDictionary *arrowOffsetDict = [[ECSThemeEngine sharedInstance] elementOfGenericType:PMThemeOffsetGenericType
                                                                                 subtype:PMThemeMainSubtype
                                                                                    type:PMThemeMonthArrowsElementType];
    ...
    if (showsLeftArrow)
    {
        ...
        [[ECSThemeEngine sharedInstance] drawPath:backArrowPath
    ...
    if (showsRightArrow)
    {
        ...
        [[ECSThemeEngine sharedInstance] drawPath:forwardArrowPath
                                  forElementType:PMThemeMonthArrowsElementType
                                         subType:PMThemeMainSubtype
                                       inContext:context];
...
{
    ECSPeriod *localPeriod = [period copy];
    if (self.allowedPeriod)
    {
...
{
    ECSPeriod* calcPeriod = [ECSPeriod oneDayPeriodWithDate:[self dateForPoint:point]];
...
- (void) periodSelectionChanged: (CGPoint) point
{
    ...
    {
        self.period = [ECSPeriod periodWithStartDate:self.period.startDate endDate:newDate];
    }
    else
    {
        self.period = [ECSPeriod oneDayPeriodWithDate:newDate];
    }
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    NSDictionary *todaySelectedBGDict = [[ECSThemeEngine sharedInstance] themeDictForType:PMThemeCalendarDigitsTodaySelectedElementType
                                                                                 subtype:PMThemeBackgroundSubtype];
    NSDictionary *inactiveSelectedDict = [[ECSThemeEngine sharedInstance] themeDictForType:PMThemeCalendarDigitsInactiveSelectedElementType
                                                                                  subtype:PMThemeMainSubtype];
    NSDictionary *todaySelectedDict = [[ECSThemeEngine sharedInstance] themeDictForType:PMThemeCalendarDigitsTodaySelectedElementType
                                                                               subtype:PMThemeMainSubtype];
    NSDictionary *activeSelectedDict = [[ECSThemeEngine sharedInstance] themeDictForType:PMThemeCalendarDigitsActiveSelectedElementType
                                                                                subtype:PMThemeMainSubtype];
    ...
    for(int i = 0; i <= weekdayOfFirst-2; i++) 
    {
        ...
        if(!self.showOnlyCurrentMonth)
        {
            [[ECSThemeEngine sharedInstance] drawString:string
			...
			if (dayNumber >= (weekdayOfFirst-1) && day <= numDaysInMonth) 
            {
                ...
                if(isToday) 
                {
                    ...
                    if (todayBGDict)
                    {
                        ...
                        UIEdgeInsets rectInset = [[[ECSThemeEngine sharedInstance] elementOfGenericType:PMThemeEdgeInsetsGenericType
                                                                                               subtype:PMThemeBackgroundSubtype
                        ...
                        [[ECSThemeEngine sharedInstance] drawPath:selectedRectPath
                                                  forElementType:type
                                                         subType:PMThemeBackgroundSubtype
                                                       inContext:context];
                    }
